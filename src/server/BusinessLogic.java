...
public class BusinessLogic implements IBusinessLogic{
		...
		while (iter.hasNext()){
			...
			if (aFile.getCreator().equals(targetID)){		//找到目标用户的文件
				if ((targetID.equals(userID))||				//若为自己文件或他人公开文件		
						aFile.getAuthorization().equals(Authorization.open)){
	...
	public DownloadFileResult downloadFile(String fileID, String userID) {
		...
		if ((!cloudFile.getCreator().equals(userID))&&	//检查是否有权限
			(cloudFile.getAuthorization().equals(Authorization.self)))
			return new DownloadFileResult();
		try {
    ...
    public AddNoteResult addNote(Note note) {
        ...
        if ((!file.getCreator().equals(userID))
                &&(!file.getAuthorization().equals(Authorization.open)))
            return AddNoteResult.wrong;
    ...
    @Override
    public AuthorizationResult setAuthorization(String fileID, String userID,
            boolean open) {
       if (!m_FileSheet.containsKey(fileID))             //文件不存在
           return AuthorizationResult.wrong;
       if (!m_FileSheet.get(fileID).getCreator().equals(userID))
           return AuthorizationResult.wrong;            //非本人文件
       if (open)
           m_FileSheet.get(fileID).setAuthorization(Authorization.open);
       else{
           m_FileSheet.get(fileID).setAuthorization(Authorization.self);
       }
       return AuthorizationResult.OK;
    }

    @Override
    public AuthorizationResult setAuthorziation(String fileID, String userID,
            HashSet<String> openList) {
        // TODO 自动生成的方法存根
        return null;
    }
}
